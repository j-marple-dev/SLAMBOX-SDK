...
#ifndef SLAMBOX_SDK_INCLUDE_SBOX_PROTOCOL_RESPONSE_RESPONSE_SERIAL_COMMUNICATION_CONFIG_HPP_
#define SLAMBOX_SDK_INCLUDE_SBOX_PROTOCOL_RESPONSE_RESPONSE_SERIAL_COMMUNICATION_CONFIG_HPP_
...
/// @brief Response serial communication config protocol
class ResponseSerialCommunicationConfig : public BaseProtocol {
 public:
  /// @brief Response serial communication config protocol constructor
  /// @param enabled enabled or not
  /// @param baudrate baudrate of serial communication
  ResponseSerialCommunicationConfig(bool enabled, uint32_t baudrate)
      : BaseProtocol(protocol::kModeResSerialCommConfig),
        enabled_(enabled),
        baudrate_(baudrate) {
  ...
  explicit ResponseSerialCommunicationConfig(const vector<uint8_t> &bytes,
                                             bool payload_only = false)
      : BaseProtocol(bytes, protocol::kModeResSerialCommConfig, payload_only) {
  ...
  void handle_callback(
    ...
    for (auto callback : callbacks) {
      callback->on_response_serial_communication_config(this->enabled_,
                                                        this->baudrate_);
    }
  }
...
#endif  // SLAMBOX_SDK_INCLUDE_SBOX_PROTOCOL_RESPONSE_RESPONSE_SERIAL_COMMUNICATION_CONFIG_HPP_
