...
#ifndef FLEXXLAM_SDK_INCLUDE_PROTOCOL_REQUEST_REQUEST_LIDAR_CONFIG_HPP_
#define FLEXXLAM_SDK_INCLUDE_PROTOCOL_REQUEST_REQUEST_LIDAR_CONFIG_HPP_
...
class RequestLidarConfigProtocol : public BaseProtocol {
 public:
  ...
  RequestLidarConfigProtocol() : BaseProtocol(protocol::kModeReqLidarConfig) {}
  ...
  explicit RequestLidarConfigProtocol(const std::vector<uint8_t> &bytes,
                                      bool payload_only = false)
      : BaseProtocol(bytes, protocol::kModeReqLidarConfig, payload_only) {}

  /// @copydoc BaseProtocol::handle_callback
  void handle_callback(
      const vector<ParsedMessageInterface *> &callbacks) override {
    for (auto callback : callbacks) {
      callback->on_request_lidar_config();
    }
  }
...
#endif  // FLEXXLAM_SDK_INCLUDE_PROTOCOL_REQUEST_REQUEST_LIDAR_CONFIG_HPP_
